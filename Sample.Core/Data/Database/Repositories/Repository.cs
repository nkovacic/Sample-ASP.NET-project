...
namespace Sample.Core.Data.Database
{
    public class Repository<TEntity> : IRepositoryAsync<TEntity> where TEntity : Entity
    {
        ...
        internal IQueryable<TEntity> Select(
            ...
            List<string> includesDotNotation = null,
            int? page = null,
            int? pageSize = null,
            bool useNoTracking = false)
        {
            ...
            if (includesDotNotation != null)
            {
                query = includesDotNotation.Aggregate(query, (current, include) => current.Include(include));
            }
        ...
        internal async Task<IEnumerable<TEntity>> SelectAsync(
            ...
            bool useNoTracking = false)
        {
            return await Select(filter, orderBy, includes, includesDotNotation, page, pageSize, useNoTracking).ToListAsync();
        }

        public virtual void DeleteGraph(TEntity entity)
        {
            SyncObjectGraph(entity);
            _entitesChecked = null;
        }

        public virtual void DeleteGraphRange(IEnumerable<TEntity> entities)
        {
            foreach (var entity in entities)
            {
                DeleteGraph(entity);
        ...
        [Obsolete("Will be renamed to UpsertGraphRange(TEntity entity) in next version.")]
